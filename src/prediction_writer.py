...
class PredictionWriter(BasePredictionWriter):
    ...
    def write_on_epoch_end(self, trainer, pl_module, predictions, batch_indices):
        ...
        confidences, word_confidences, margins, word_margins, img_embeddings, img_ids, predicted_tokens = list(
            zip(*predictions[0])
        )
        img_ids = torch.cat(img_ids, axis=0)
        if "confidence" in self.strategy:
            confidences = torch.cat(confidences, axis=0)
            word_confidences = torch.cat(word_confidences, axis=0)
            margins = torch.cat(margins, axis=0)
            word_margins = torch.cat(word_margins, axis=0)
        ...
        word_confidences_path = self.current_dir / f"word_confidences_rank_{trainer.global_rank}"
        margins_path = self.current_dir / f"margins_rank_{trainer.global_rank}"
        word_margins_path = self.current_dir / f"word_margins_rank_{trainer.global_rank}"
        img_embedding_path = (
        ...
        torch.save(word_confidences, str(word_confidences_path))
        torch.save(margins, str(margins_path))
        torch.save(word_margins, str(word_margins_path))
